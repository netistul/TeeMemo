...
export const deleteNote = async (
  ...
  try {
    AsyncStorage.setItem(
      "notes",
      JSON.stringify(notes.filter((note) => note.id !== noteToDeleteId))
    );
  } catch (error) {
...
export const DeleteButtons = ({
  ...
  <>
    <Button mode="contained" buttonColor="#ed3b5a" onPress={handleBulkDelete}>
      {`Delete ${selectedNotes.size} selected notes`}
    </Button>

    <Button mode="outlined" onPress={exitBulkDeleteMode}>
      Cancel
    </Button>
  </>
);
